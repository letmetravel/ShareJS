...
Connection.prototype.handleMessage = function(msg) {
  ...
  switch (msg.a) {
    ...
    case 'ra':
      var action = this.actions[msg.id];

      if (msg.error) {
        return action.callback(msg.error);
      }
      action.callback(null, msg.result);
      break;

    case 'opr':
      var sb = this.rawSubmits[msg.id];

      if (msg.error) {
        return sb.callback(msg.error);
      }
      sb.callback(null, msg.v, msg.ops);
      break;

    case 'getOps':
      var task = this.getOpsTasks[msg.id];

      if (msg.error) {
        return task.callback(msg.error);
      }
      task.callback(null, msg.ops);
      break;
 ...
 * Run action.
 */
Connection.prototype.runAction = function(actionName, collection, docName, options, callback) {
  if (!options) options = {};
  var id = this.nextActionId++;
  this.actions[id] = {callback: callback, c: collection, d: docName, o: options, n: actionName};
  this.send({a:'ra', n: actionName, o: options, c: collection, d: docName, id: id})
};


/**
 * Send raw operation.
 * отправить операция не через документ а напрямую
 */
Connection.prototype.submitOpRaw = function(collection, docName, opData, callback) {
  var id = this.nextRawSubmitId++;
  this.rawSubmits[id] = {callback: callback, c: collection, d: docName, opData: opData};
  this.send({a:'opr', opData: opData, c: collection, d: docName, id: id})
};

/**
 * Получить список операций
 */
Connection.prototype.getOps = function(collection, docName, from, to, callback) {
  var id = this.nextGetOpsId++;
  this.getOpsTasks[id] = {callback: callback, c: collection, d: docName, from: from, to: to};
  this.send({a:'getOps', c: collection, d: docName, from: from, to: to, id: id})
};
