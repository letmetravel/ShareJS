...
module.exports = function(instance, stream, req) {
  var session = new Session(instance, stream, req);
 ...
 * @param {Http.Request} req
 */
function Session(instance, stream, req) {
  ...
  // мы хотим при init отдавать id текущего пользователя
  var userId = (req && req.user) ? req.user.id : null;

  // Initialize the remote client by sending it its session Id.
  this._send({a:'init', protocol:0, id:this.agent.sessionId, userId: userId});
}
...
Session.prototype._checkRequest = function(req) {
  if (req.a === 'qsub' || req.a === 'qfetch' || req.a === 'qunsub') {
    ...
    // Run action
    if (!req.n || typeof req.n !== 'string') return 'Invalid actionName';
    if (!req.collection) return 'Invalid docName or collection';
  } else if (req.a === 'getOps') {
    // Run action
    if (!req.from || typeof req.from !== 'string') return 'Invalid from value';
    if (!req.to || typeof req.to !== 'string') return 'Invalid to value';
  } else {
...
Session.prototype._handleMessage = function(req, callback) {
  ...
  switch (req.a) {
    ...
    case 'opr':
      // Тоже самое что и submit operation,
      // только возвращает ответ сразу, а не шлет операции по одной
      // удобно для рест апи
      var opData = req.opData;
    ...
    case 'getOps':
      ...
      agent.getOps(collection, docName, from, to, function(err, ops) {
        if (err) {
          console.trace(err);
        }
        if (err) return callback(err);

        callback(null, {ops: ops});
      });
      break;
    ...
    case 'ra':
      ...
      agent.runAction(name, collection, docName, options, function(err, result) {
        if (err) return callback(err);
