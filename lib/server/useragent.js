...
UserAgent.prototype.submit = function(collection, docName, opData, options, callback) {
  ...
  agent.trigger('submit', collection, docName, {opData: opData, channelPrefix:null}, function(err, action) {
    ...
    if (
      agent.user &&
      agent.user.id &&
      typeof agent.user.id === 'string'
    ) {
      if (typeof opData.m !== 'object') {
        opData.m = {};
      }
      opData.m.userId = agent.user.id;
    }
 ...
 * Run action
 */
UserAgent.prototype.runAction = function(name, collection, docName, options, callback) {
  var agent = this;

  agent.trigger('action', collection, docName, {options: options, name:name}, function(err, action) {
    if (err) return callback(err);

    if (!action.actionHandler) {
      return callback('Cannot find action');
    }

    action.actionHandler(name, collection, docName, options, callback);
  });
};
